...
const imgCps = document.querySelector(".img-cps");
const servicesText = document.querySelector(".services_text");
const serviceParagraph = document.querySelector(".service-text");
...
readMoreBtn.addEventListener("click", () => {
  ...
  if (isHidden) {
    ...
    // Перемещаем картинку вниз ЗА ВСЁМ содержимым, только если ширина 430px
    if (window.innerWidth === 430) {
      servicesText.appendChild(imgCps);
    }
  } else {
    moreText.style.display = "none";
    ...
    // Возвращаем картинку обратно ПЕРЕД основным текстом, только если ширина 430px
    if (window.innerWidth === 430) {
      servicesText.insertBefore(imgCps, serviceParagraph);
    }
  }
});