...
const contents = document.querySelectorAll('.services__content');
  ...
  const cpsImage = document.querySelector(".container-picture");

  function isMobileView() {
    return window.innerWidth <= 430;
  }

  function resetImageOnResize() {
    if (!isMobileView()) {
      // Удаляем мобильный класс при расширении окна
      cpsImage.classList.remove("move-down");
    }
  }
  ...
  readMoreBtn.addEventListener("click", () => {
    ...
    if (moreText.style.display === "none") {
      ...
      if (isMobile) {
        cpsImage.classList.add("move-down");
      }
  ...
  window.addEventListener("resize", resetImageOnResize);
